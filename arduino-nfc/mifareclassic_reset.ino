...
static const uint8_t KEY_KEYAB[6] = {0xD3, 0xF7, 0xD3, 0xF7, 0xD3, 0xF7};
...
void loop(void) {
  ...
  if (success){
    // We seem to have a tag . Display some basic information about it
    ...
    for (idx = 0; idx < numOfSector; idx++){
      // Step 1: Authenticate the current sector using key B 0xFF 0xFF 0xFF 0xFF 0xFF 0xFF --> 0xD3, 0xF7, 0xD3, 0xF7, 0xD3, 0xF7
//      success = nfc.mifareclassic_AuthenticateBlock (uid, uidLength, BLOCK_NUMBER_OF_SECTOR_TRAILER(idx), 1, (uint8_t *)KEY_KEYAB);
//      if (!success){
...
//      }
      if ((idx == 0)){// || (idx == 16)
        ...
        if (!(nfc.mifareclassic_WriteDataBlock((BLOCK_NUMBER_OF_SECTOR_TRAILER(idx)) - 2, blockBuffer))){
          Serial.print("Unable to write to sector 0? or"); Serial.println(numOfSector);
          return;
        }
      }
//      else if((idx == 1) || (idx == 2)){
//        success = nfc.mifareclassic_AuthenticateBlock (uid, uidLength, BLOCK_NUMBER_OF_SECTOR_TRAILER(idx), 0, (uint8_t *)keya2);
//        if (!success){
//          Serial.print("Authentication failed for sector -->"); Serial.println(idx);
//          return;
//        }
